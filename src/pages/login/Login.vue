        ...
        <form @submit.prevent="login">
          ...
          <div :class="{on: !loginWay}">
            <section>
              ...
              <section class="login_verification">
                <input type="text" maxlength="8" placeholder="密码" v-if="isShowPwd" v-model="pwd"/>
...
export default {
  ...
  computed: {
    rightPhone() {
      const regXP = /^1[3456789]\d{9}$/
      return regXP.test(this.phone)
    }
  },
  methods: {
    async getCode() {
    ...
    async login() {
      ...
      if(res.code === 0) {
        const user = res.data
        //将user信息保存到state
        this.$store.dispatch('userRecord', user)
        //去往个人中心界面
